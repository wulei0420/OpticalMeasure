...
def match_side(cx, cy, side, ref_disp=None):
    if side == 'left':
        ...
        base_disp, margin = 374, 40  # narrower for right since constrained by left
...
def match_and_tri(cx, cy):
    ...
    if left and right:
        dl = abs(left['x'] - cx); dr = abs(cx - right['x'])
        # Apply feedback-learned correction factors (from 160 samples)
        dl = dl * 1.03   # left: feedback-adjusted
        dr = dr * 1.40   # right: feedback-adjusted (larger correction)
        Z_lc = FX * BL_LC / dl if dl > 0 else 1000
        Z_cr = FX * BL_CR / dr if dr > 0 else 1000
        Z = (Z_lc + Z_cr) / 2
        X = (cx - cw/2) * Z / FX
        Y = (cy - ch/2) * Z / FX
        return left, right, [round(X,1), round(Y,1), round(Z,1)], round(abs(Z_lc - Z_cr), 1)
...
def preview():
    frm = get_preview()
    if frm is None: return 'Camera error', 500
